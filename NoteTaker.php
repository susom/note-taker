...
class NoteTaker extends \ExternalModules\AbstractExternalModule
{
    ...
    public function redcap_save_record($project_id, $record = NULL, $instrument, $event_id, $group_id = NULL, $survey_hash, $response_id, $repeat_instance = 1)
    {
        ...
        foreach ($instances as $i => $instance) {
            ...
            // Only process this config if the form just saved matches the instance event id
            if ($i_event_id == $event_id) {

                // Load the fields on the instrument
                ...
                if (in_array($i_input_field, $instrument_fields)) {
                    ...
                    // Check if the input-field is empty after the last save
                    $fields = [
                        $i_input_field,
                        $i_date_field,
                        $i_note_field
                    ...
                    $this->emDebug("Record $record current data", $data);
                    ...
                    if (!empty($data[$i_input_field])) {
                        ...
                        // Update note box with the latest note
     ...
     *  Example call -
     *  $data = array(), $i_note_field = "notebox1" , $i_input_field = "Append Me", $i_date_field = "dateTime1",
     * $i_include_delimiter = True return - data[notebox1] = "[<username> @ <dateNow> ] \n ---------------------- \n
     * <previous note> \n
     *
